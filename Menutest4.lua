...
local CONFIG = {
    GUI_NAME = "CafeinaTRexDirectCharacterV51",
    ...
    -- V5.1: o Humanoid é usado para Character/câmera, mas o deslocamento
    -- é feito diretamente pelo HumanoidRootPart ancorado.
    TURN_RESPONSE = 15,
    MAX_DT = 0.05,

    GROUND_PROBE_UP = 12,
    GROUND_PROBE_DOWN = 34,
    MAX_GROUND_CHANGE = 7,
    BLOCK_VOID = true,

    WALL_COLLISION = true,
    WALL_PROBE_HEIGHT = 1.7,
    WALL_PADDING = 0.45,

    -- Pulo visual/local independente da física do Humanoid.
    JUMP_VELOCITY = 17,
    GRAVITY = 46,
...
local function prepareCharacter(model)
    ...
    -- V5.1: controller root stays anchored and is moved by CFrame.
    -- Everything welded to it follows, while Motor6D/Bones remain free to animate.
...
--==============================================================--
-- DIRECT-CONTROL GROUND / WALL HELPERS
--==============================================================--

local function directRayParams()
    ...
    if State.DriverCharacter then
        table.insert(ignore, State.DriverCharacter)
    end

    if State.DinoCharacter then
        table.insert(ignore, State.DinoCharacter)
    end
...
local function stableGroundAt(x,z)
    ...
    local result =
        Workspace:Raycast(
            Vector3.new(
                x,
                State.GroundY + CONFIG.GROUND_PROBE_UP,
                z
            ),
            Vector3.new(
                0,
                -(CONFIG.GROUND_PROBE_UP + CONFIG.GROUND_PROBE_DOWN),
                0
            ),
            directRayParams()
        )
    ...
    if math.abs(y - State.GroundY) > CONFIG.MAX_GROUND_CHANGE then
        return nil,nil
    end
...
local function directWallBlocked(fromX,fromZ,toX,toZ)
    ...
    local result =
        Workspace:Raycast(
            Vector3.new(
                fromX,
                State.GroundY + CONFIG.WALL_PROBE_HEIGHT,
                fromZ
            ),
            delta.Unit * (
                delta.Magnitude + CONFIG.WALL_PADDING
            ),
            directRayParams()
        )
...
local function placeDirectRoot()
    if not State.DinoRoot
    or not State.GroundY
    then
        return
    end
...
local function startControlLoop()
    ...
    State.RenderConn =
        ...
        Connect(function(dt)
            ...
            local world =
                active
                and inputToWorld(raw)
                or Vector3.zero

            -- V5.1 GUARANTEE:
            -- VirtualX/VirtualZ only change inside this block, therefore
            -- no joystick input means no backward/automatic movement.
            if active
            and world.Magnitude > 0.001
            then
                local desired = world.Unit
                ...
                then
                    State.VirtualX = nextX
                    State.VirtualZ = nextZ
                ...
                -- Standing still can refine the local floor height,
                -- but cannot modify X/Z.
                local floor =
                    stableGroundAt(
                        State.VirtualX,
                        State.VirtualZ
            ...
            -- Independent visual jump.
            if not State.Grounded then
                State.VerticalVelocity =
                    State.VerticalVelocity
                    - CONFIG.GRAVITY * frameDt

                State.VerticalOffset =
                    State.VerticalOffset
                    + State.VerticalVelocity * frameDt

                if State.VerticalOffset <= 0 then
...
local function enableMorph()
    ...
    -- V5.1 direct controller starts exactly where the ground-aligned
    -- Character root was placed.
    State.VirtualX =
        prepared.root.Position.X

    State.VirtualZ =
        prepared.root.Position.Z

    State.GroundY =
        prepared.root.Position.Y
        - prepared.root.Size.Y * 0.5
    ...
    local startLook =
        Vector3.new(
            spawnCF.LookVector.X,
            0,
            spawnCF.LookVector.Z
        )

    if startLook.Magnitude > 0.001 then
        State.LastLook =
            startLook.Unit
    else
        State.LastLook =
    ...
    -- Root remains anchored; CFrame controller owns movement.
    prepared.root.Anchored = true
    prepared.root.CanCollide = false
...
Title.Text = "CAFEÍNA • T-REX CHARACTER V5.1"
...
Subtitle.Text = "DIRECT CFRAME CONTROL • MOBILE"
...
Status.Text = "Pronto • movimento direto pelo root • sem auto-walk"
...
Connect(function()
    ...
    if State.Enabled then
        ...
        if ok then
            ...
            setStatus(
                "T-Rex V5.1 ativo • joystick move o root diretamente"
            )
        else
            Action.Text =
                "VIRAR T-REX"
...
print("[CAFEÍNA] T-REX DIRECT CHARACTER V5.1 carregado.")
