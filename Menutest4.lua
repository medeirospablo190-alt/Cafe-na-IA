...
local CONFIG = {
    VERSION = "TREX_EXECUTOR_MORPH_V3_1",
    GUI_NAME = "CafeinaTRexExecutorMorphV31",
    ...
    -- V3.1: movimento só é considerado voluntário quando existe
    -- input real do PlayerModule/joystick.
    INPUT_DEADZONE = 0.08,
...
local function getRealMoveInput()
    ...
    -- Fallback only when the standard PlayerModule is unavailable.
    -- This is less reliable because game scripts can manipulate MoveDirection.
...
local function startFollowLoop()
    ...
    State.FollowConn =
        ...
        Connect(function(dt)
            ...
            -- If the game calls Humanoid:MoveTo() or leaves residual movement,
            -- cancel it locally while the user is not touching controls.
            if CONFIG.MANUAL_CONTROL_ONLY
            and State.ControlsAvailable
            and not inputActive
            then
                pcall(function()
            ...
            -- Humanoid.MoveDirection is still useful as WORLD direction,
            -- but only after real joystick/keyboard input authorizes movement.
            local move = hum.MoveDirection
            ...
            -- V3.1: Idle/Walk/Run is gated by REAL input, not by residual
            -- Humanoid velocity or MoveTo commands from game scripts.
            ...
            elseif speed >= CONFIG.RUN_SPEED_THRESHOLD then
                ...
                playTrack(
                    "walk",
                    math.clamp(
                        math.max(speed, CONFIG.WALK_MIN_SPEED)
                        / math.max(
                            hum.WalkSpeed,
                            1
                        ),
...
Title.Text="CAFEÍNA • T-REX EXECUTOR V3.1"
...
Status.Text="Pronto • movimento só aceita joystick/WASD real"
...
print("[CAFEÍNA] T-REX EXECUTOR MORPH V3.1 carregado.")
