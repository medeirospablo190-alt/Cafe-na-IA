...
-- BOOT VISUAL
--==============================================================--
local oldBoot = GuiParent:FindFirstChild("CafeinaBootV34")
if oldBoot then
    oldBoot:Destroy()
end

local BootGui = Instance.new("ScreenGui")
BootGui.Name = "CafeinaBootV34"
...
BootLabel.Size = UDim2.fromOffset(180, 34)
BootLabel.Position = UDim2.new(0.5, -90, 0, 18)
BootLabel.BackgroundColor3 = Color3.fromRGB(65, 18, 25)
BootLabel.BorderSizePixel = 0
BootLabel.Text = "CAFEÍNA • iniciando..."
...
BootCorner.CornerRadius = UDim.new(0, 9)
...
task.spawn(function()
    ...
    if not ok then
        warn("[CAFEINA V3.4] Falha inicial ao vincular personagem:", err)
    end
end)
...
Connect(Player.CharacterAdded, function(character)
    task.spawn(function()
        ...
        if not ok then
            warn("[CAFEINA V3.4] Falha no respawn:", err)
        end
    end)
end)
...
local function ParsePrice(text)
    ...
    if suffix and suffix ~= "" then
        value = value * (multipliers[suffix] or 1)
    end
...
for _, name in ipairs({"CafeinaTycoonV2", "CafeinaTycoonV25", "CafeinaTycoonV3", "CafeinaTycoonV31", "CafeinaTycoonV34"}) do
...
if BootLabel and BootLabel.Parent then BootLabel.Text = "CAFEÍNA • menu carregado" end
...
Pages.Scanner = function(version)
    ...
    task.spawn(function()
        ...
        LatestReport = {
            generatedBy = "CAFEINA TYCOON V3.4",
            player = Player.Name,
...
task.delay(0.12, function()
    ...
    if not ok then
        warn("[CAFEÍNA V3.4] Falha ao abrir Principal:", err)
    end
end)

task.delay(0.8, function()
    if BootGui and BootGui.Parent and GUI and GUI.Parent then
        BootGui:Destroy()
    end
end)

print("[CAFEÍNA V3.4] GUI criada; carregamento assíncrono iniciado")
