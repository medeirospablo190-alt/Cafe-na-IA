...
-- BOOT VISUAL
--==============================================================--
local oldBoot = PlayerGui:FindFirstChild("CafeinaBootV33")
if oldBoot then
    oldBoot:Destroy()
end

local BootGui = Instance.new("ScreenGui")
BootGui.Name = "CafeinaBootV33"
...
task.spawn(function()
    ...
    if not ok then
        warn("[CAFEINA V3.3] Falha inicial ao vincular personagem:", err)
    end
end)
...
Connect(Player.CharacterAdded, function(character)
    task.spawn(function()
        ...
        if not ok then
            warn("[CAFEINA V3.3] Falha no respawn:", err)
        end
    end)
end)
...
for _, name in ipairs({"CafeinaTycoonV2", "CafeinaTycoonV25", "CafeinaTycoonV3", "CafeinaTycoonV31", "CafeinaTycoonV33"}) do
...
Pages.Scanner = function(version)
    ...
    task.spawn(function()
        ...
        LatestReport = {
            generatedBy = "CAFEINA TYCOON V3.3",
            player = Player.Name,
...
task.delay(0.12, function()
    ...
    if not ok then
        warn("[CAFEÍNA V3.3] Falha ao abrir Principal:", err)
    end
end)
...
print("[CAFEÍNA V3.3] GUI criada; carregamento assíncrono iniciado")
