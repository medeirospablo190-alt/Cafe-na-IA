...
-- Manual legitimate drop -> immediate pickup -> ONE same-instance retry
-- Focused/event-driven • no hook • no spatial scan • no remote flood
--==============================================================
...
local function safe(v, depth)
    ...
    if t == "string" then
        return #v <= 500 and v or string.sub(v, 1, 500) .. "...[truncated]"
    end
...
local function addError(phase, message)
    if #S.errors < CFG.MAX_ERRORS then
        S.errors[#S.errors + 1] = {
            phase=tostring(phase), error=tostring(message), clock=os.clock(), unix=os.time()
        }
    end
end
...
local function chooseItem()
    -- Equipped crystal wins, so the user chooses by equipping it before arming.
    ...
    for _, container in ipairs(containers()) do
        if container then
            for _, obj in ipairs(container:GetChildren()) do
                if obj:IsA("Tool") and tostring(obj:GetAttribute("BagId")) == tostring(id) then
                    return obj
                end
            end
        end
...
local function activeToolState(item)
    local bagSet, bagCount, toolCount = {}, 0, 0
    for _, container in ipairs(containers()) do
        if container then
            for _, obj in ipairs(container:GetChildren()) do
                if sameTool(obj, item) then
                    ...
                    if id ~= nil and not bagSet[tostring(id)] then
                        bagSet[tostring(id)] = true
                        bagCount += 1
                    end
                end
            end
    ...
    return toolCount, bagCount, bagSet
end

local function countToolsByBagId(id)
    local count = 0
    for _, container in ipairs(containers()) do
        if container then
            for _, obj in ipairs(container:GetChildren()) do
...
local function buildSummary()
    ...
    local toolCount, bagCount, bagSet = 0, 0, {}
    if item then toolCount, bagCount, bagSet = activeToolState(item) end
    ...
    S.test.timelineMs = {
        ...
        firstPromptFire_to_firstInventoryChanged=ms(m.firstPromptFire,m.firstInventoryChanged),
    ...
    S.test.finalState = {
        ...
        activeMatchingTools=toolCount,
        activeMatchingBagIds=bagCount,
        activeBagIds=bagSet,
        uid=S.test.uid,
    ...
    if item and S.test.firstNewBagId and tostring(S.test.firstNewBagId) ~= tostring(item.bagId) and findBagId(item.bagId) then
        signal("OLD_AND_FIRST_NEW_BAGID_COEXIST", "warning", {oldBagId=item.bagId,newBagId=S.test.firstNewBagId})
    end
    if S.test.uid and uidWorldCount(S.test.uid) > 1 then
        signal("SAME_UID_MULTIPLE_WORLD_OBJECTS", "critical", {uid=S.test.uid,count=uidWorldCount(S.test.uid)})
    end
    if toolCount > 1 then
        signal("MULTIPLE_MATCHING_TOOLS_ACTIVE", "critical", {toolCount=toolCount,bagIds=bagSet})
    end
end
        ...
        trace={
            ...
            diagnostics={
                ...
                guards={
                    ...
                    firstRequiresToolInventoryAndGemCollected=true,
                    secondUsesSameWorldInstance=true,
...
local function scheduleSecondFinish()
    if S.secondFinishScheduled or S.finalized then return end
    S.secondFinishScheduled = true
    task.delay(CFG.SECOND_OBSERVE, function()
        if S.active and not S.finalized then finish("second_attempt_observed") end
    end)
end

local doSecondAttempt

local function maybeConfirmFirstPickup()
    if not S.active or S.finalized or S.firstPickupConfirmed then return end

    -- IMPORTANT: require ALL three first-pickup signals before opening the second window.
    -- This prevents a late first GemCollected from being misclassified as a second response.
    if S.marks.firstToolAdded and S.marks.firstInventoryChanged and S.marks.firstGemCollected then
...
doSecondAttempt = function()
    ...
    if (not prompt or not prompt.Parent) and crystal and crystal.Parent then
        prompt = findPrompt(crystal)
    end
    ...
    S.test.secondAttempt = {
        ...
        uid=S.test.uid,
        firstNewBagId=S.test.firstNewBagId,
        firstNewBagIdPresent=S.test.firstNewBagId and findBagId(S.test.firstNewBagId) ~= nil or false,
    }
...
local function attemptFirstPrompt(prompt)
    ...
    rec({
        kind="first_pickup_prompt_ready",
        prompt=pathOf(prompt),
...
local function confirmWorldCrystal(crystal)
    ...
    -- Tolerate a rare replication reorder where world arrives before old Tool removal is visible.
...
local function installWatchers()
    local function watchContainer(container, label)
        ...
        local added = container.ChildAdded:Connect(function(obj)
            ...
            -- Backpack <-> Character transfer of the ORIGINAL Tool is not a drop.
            ...
            if S.secondAttempted then
                local firstBag = S.test.firstNewBagId
                if tostring(bagId) == tostring(firstBag) then
                    local copies = countToolsByBagId(firstBag)
                    rec({kind="existing_first_tool_added_during_second_window",container=label,bagId=bagId,copies=copies})
                    if copies > 1 then
                        S.marks.secondToolAdded = S.marks.secondToolAdded or os.clock()
                        S.counters.secondToolAdded += 1
                        signal("DUPLICATE_TOOL_SAME_BAGID_AFTER_SECOND", "critical", {bagId=bagId,copies=copies})
                    end
                    return
                end
                ...
                if not S.marks.secondToolAdded then
                    ...
                    signal("SECOND_ATTEMPT_CREATED_NEW_TOOL", "critical", {firstBagId=firstBag,secondBagId=bagId})
                end
                return
            end
        ...
        local conn = InventoryChanged.OnClientEvent:Connect(function(...)
            ...
            if S.secondAttempted then
                ...
                if not S.marks.secondInventoryChanged then
                    ...
                    signal("SECOND_ATTEMPT_TRIGGERED_INVENTORY_CHANGED","warning",{arguments=args})
                end
            elseif S.firstPromptFired then
                stage = "after_first_attempt"
                S.marks.firstInventoryChanged = S.marks.firstInventoryChanged or now
            else
        ...
        local conn = GemCollected.OnClientEvent:Connect(function(crystal,player,...)
            ...
            if S.secondAttempted then
                ...
                if not S.marks.secondGemCollected then
                    ...
                    signal("SECOND_ATTEMPT_TRIGGERED_GEM_COLLECTED","warning",{uid=S.test.uid})
                end
            else
                stage = "after_first_attempt"
...
-- Stop older CAFEINA collectors/tests so only this lightweight test observes the run.
for _, key in ipairs({
...
task.spawn(function()
    while gui.Parent do
        local candidate
        if not S.active and not S.sending then candidate=chooseItem() end
