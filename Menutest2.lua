--==============================================================--
-- CAFEÍNA • MORPH PASSIVE TRACE V6.0
...
-- NÃO instala hooks de namecall.
-- NÃO tenta burlar unlocks.
-- NÃO faz brute force.
-- Só observa o fluxo legítimo do jogo.
--==============================================================--
...
local CONFIG = {
    VERSION = "MORPH_PASSIVE_TRACE_V6_0",
    GUI_NAME = "CafeinaMorphPassiveTraceV60",
    ...
    ARCHIVE_ROOT = "CafeinaMorphPassiveV6",
    ARCHIVE_FOLDER = "CafeinaMorphPassiveV6/" .. tostring(game.PlaceId),
    ARCHIVE_FILE = "CafeinaMorphPassiveV6/" .. tostring(game.PlaceId) .. "/trace.jsonl",
...
pcall(function()
    local old = rawget(env, "__CAFEINA_MORPH_SUCCESS_TRACE_V4")
...
local function startTrace()
    ...
    appendRecord({
        ...
        purpose="capture legitimate successful morph without interfering with the native flow",
    })
    ...
    if S.UI.Status then
        S.UI.Status.Text="OBSERVANDO • transforme normalmente pelo menu original do jogo"
    end
end
    ...
    local sOK,sData,sErr=postJson(
        CONFIG.UPLOAD_BASE.."/start",
        {
            filename=string.format(
                "Cafeina_MorphPassiveV6_%s_%s.json",
...
Title.Text="CAFEÍNA • MORPH PASSIVE TRACE V6"
...
Sub.Text="ZERO-HOOK • 2s ANTES • 5s DEPOIS"
...
Counts.Text="Attrs 0 • MorphApplied 0 • Character 0"
...
local function refreshUI()
    ...
    Counts.Text=string.format(
        "Attrs %d • MorphApplied %d • Character %d",
        S.Counters.morphAttrChanged,
...
env.__CAFEINA_MORPH_PASSIVE_V6={
    Start=startTrace,
    Stop=stopTrace,
    Upload=uploadAll,
...
print("[CAFEÍNA] MORPH PASSIVE TRACE V6.0 ZERO-HOOK carregado.")
