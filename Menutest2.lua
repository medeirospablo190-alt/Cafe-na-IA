--==============================================================--
-- CAFEÍNA • EGG DELIVERY TRACE V1.6 • PASSIVE SAFE
--
...
local CONFIG = {
    VERSION = "EGG_DELIVERY_TRACE_V1_6_PASSIVE_SAFE",
    GUI_NAME = "CafeinaEggDeliveryTraceV16",
...
pcall(function()
    for _, key in ipairs({
        "__CAFEINA_EGG_TRACE_V16_CONTROLLER",
...
Title.Text = "CAFEÍNA • EGG TRACE V1.6"
...
local function buildUploadChunks()
    ...
    iterateArchiveObjects(function(object)
...
local function uploadAll()
    ...
    StatusLabel.Text = "Preparando arquivo..."
    updateUI(true)

    local okBuild, chunks, totalBytes =
        pcall(buildUploadChunks)
    ...
    then
        ...
        Upload.LastError = tostring(chunks)

        StatusLabel.Text = "Falha ao preparar • preservado"
    ...
    -- Exact Menutest-style /start contract.
    ...
    if not startOk then
        ...
        StatusLabel.Text = "Erro /start • preservado"
        ActionButton.Text = "REENVIAR ARQUIVO"
        ActionButton.BackgroundColor3 = COLORS.BUTTON
        updateUI(true)
        return
    ...
    if not Upload.UploadId then
        ...
        StatusLabel.Text = "/start inválido • preservado"
        ActionButton.Text = "REENVIAR ARQUIVO"
        ActionButton.BackgroundColor3 = COLORS.BUTTON
        updateUI(true)
        return
    ...
    for index, objects in ipairs(chunks) do
        ...
        local chunkBytes = #safeJson(objects)

        -- Exact Menutest-style /chunk contract.
        local chunkOk, _, chunkErr =
            httpPostJson(
        ...
        if not chunkOk then
            ...
            StatusLabel.Text = "Erro no chunk • preservado"
            ActionButton.Text = "REENVIAR ARQUIVO"
            ActionButton.BackgroundColor3 = COLORS.BUTTON
            updateUI(true)
            return
    ...
    -- Exact Menutest-style /finish contract.
    ...
    if not finishOk then
        ...
        StatusLabel.Text = "/finish falhou • preservado"
        ActionButton.Text = "REENVIAR ARQUIVO"
        ActionButton.BackgroundColor3 = COLORS.BUTTON
        updateUI(true)
        return
    ...
    if not confirmed then
        ...
        StatusLabel.Text = "Servidor não confirmou • preservado"
        ActionButton.Text = "REENVIAR ARQUIVO"
        ActionButton.BackgroundColor3 = COLORS.BUTTON
        updateUI(true)
        return
    ...
    StatusLabel.Text = "Upload confirmado ✓"
...
env.__CAFEINA_EGG_TRACE_V16_CONTROLLER = {
    Gui=Gui,
...
print(
    "[CAFEÍNA] EGG DELIVERY TRACE V1.6 PASSIVE SAFE carregado."
)
