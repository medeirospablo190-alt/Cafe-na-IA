...
--==============================================================--
-- CAFEÍNA ASSISTENTE • LAUNCHER
--==============================================================--

local AssistantLauncher = Instance.new("TextButton")
AssistantLauncher.Size = UDim2.new(1, -14, 0, 39)
AssistantLauncher.Position = UDim2.fromOffset(7, 62)
...
AssistantLauncher.Text = ""
corner(AssistantLauncher, 9)
stroke(AssistantLauncher, COLORS.RED_DARK, 1)
AssistantLauncher.Parent = Main

local AssistantIcon = label(AssistantLauncher, "✦", 16, COLORS.RED)
AssistantIcon.Font = Enum.Font.GothamBold
AssistantIcon.Size = UDim2.fromOffset(37, 39)
AssistantIcon.Position = UDim2.fromOffset(4, 0)
AssistantIcon.TextXAlignment = Enum.TextXAlignment.Center

local AssistantName = label(AssistantLauncher, "CAFEÍNA ASSISTENTE", 11, COLORS.TEXT)
AssistantName.Font = Enum.Font.GothamBold
AssistantName.Size = UDim2.new(1, -86, 0, 20)
AssistantName.Position = UDim2.fromOffset(42, 2)

local AssistantHint = label(
    AssistantLauncher,
    "Pergunte sobre o Explorer ou o arquivo aberto",
    8,
    COLORS.SUB
)
AssistantHint.Size = UDim2.new(1, -86, 0, 16)
AssistantHint.Position = UDim2.fromOffset(42, 20)
AssistantHint.TextTruncate = Enum.TextTruncate.AtEnd

local AssistantArrow = label(AssistantLauncher, "›", 18, COLORS.SUB)
AssistantArrow.Size = UDim2.fromOffset(32, 39)
AssistantArrow.Position = UDim2.new(1, -36, 0, 0)
AssistantArrow.TextXAlignment = Enum.TextXAlignment.Center
...
PathBar.Position = UDim2.fromOffset(7, 108)
...
Search.Position = UDim2.fromOffset(7, 152)
...
Content.Size = UDim2.new(1, -14, 1, -251)
Content.Position = UDim2.fromOffset(7, 197)
...
AIHeader.Size = UDim2.new(1, 0, 0, 55)
...
AIBack.Position = UDim2.fromOffset(8, 9)

local AIHeaderIcon = label(AIHeader, "✦", 16, COLORS.RED)
AIHeaderIcon.Font = Enum.Font.GothamBold
AIHeaderIcon.Size = UDim2.fromOffset(34, 55)
AIHeaderIcon.Position = UDim2.fromOffset(48, 0)
...
AITitle.Size = UDim2.new(1, -132, 0, 24)
AITitle.Position = UDim2.fromOffset(82, 5)

local AISubtitle = label(AIHeader, "Especialista no Explorer", 8, COLORS.SUB)
AISubtitle.Size = UDim2.new(1, -132, 0, 18)
AISubtitle.Position = UDim2.fromOffset(82, 29)
AISubtitle.TextTruncate = Enum.TextTruncate.AtEnd

local AIClear = button(AIHeader, "LIMPAR")
AIClear.Size = UDim2.fromOffset(61, 31)
AIClear.Position = UDim2.new(1, -68, 0, 12)
AIClear.TextSize = 9

local AIChat = Instance.new("ScrollingFrame")
AIChat.Size = UDim2.new(1, -10, 1, -128)
AIChat.Position = UDim2.fromOffset(5, 60)
...
AIComposer.Size = UDim2.new(1, -10, 0, 60)
AIComposer.Position = UDim2.new(0, 5, 1, -64)
...
AIPrompt.Size = UDim2.new(1, -61, 1, -10)
...
AISend.Size = UDim2.fromOffset(46, 46)
AISend.Position = UDim2.new(1, -51, 0.5, -23)
...
local function setAIVisible(visible)
    ...
    Top.Visible = not AIVisible
    AssistantLauncher.Visible = not AIVisible
    PathBar.Visible = not AIVisible
    Search.Visible = not AIVisible and CurrentMode == "LIST"
    Content.Visible = not AIVisible
    Bottom.Visible = not AIVisible

    if AIVisible then
        AISubtitle.Text =
            SelectedPath
            and ("Arquivo atual: " .. basename(SelectedPath))
            or ("Pasta atual: " .. (CurrentPath == "" and "/" or CurrentPath))
...
local function sendAI(question)
    ...
    if not API.request then
        addChatMessage(
            "assistant",
            "Não encontrei request/http_request/syn.request neste ambiente. "
            .. "Sem uma dessas APIs não consigo acessar o backend da IA."
        )
        return
    end
    ...
    task.spawn(function()
        ...
        if not answer or answer == "" then
            addChatMessage(
                "assistant",
                "O backend respondeu, mas não encontrei texto utilizável."
            )
            return
        end
