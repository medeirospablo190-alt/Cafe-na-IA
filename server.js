...
app.get("/api/diagnostic-sources/:id/loader.lua", async (req, res) => {
  try {
    const id = safeName(req.params.id, "");
    const sources = await readSources();
    const source = sources.find((item) => item.id === id);
    if (!source) return res.status(404).type("text/plain").send("-- Fonte removida ou não encontrada.\n");

    res.set("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate");
    res.set("Pragma", "no-cache");
    res.set("Expires", "0");
    return res.type("text/plain; charset=utf-8").send(monitoredLoaderLua(req, source));
  } catch (error) {
    console.error("diagnostic-sources/loader", error);
    return res.status(500).type("text/plain").send("-- Não foi possível gerar o loader monitorado.\n");
  }
});

app.post("/api/diagnostic-sources/:id/runtime-report", async (req, res) => {
  try {
    const id = safeName(req.params.id, "");
    const sources = await readSources();
    const source = sources.find((item) => item.id === id);
    ...
    const reportToken = String(req.body?.reportToken || "");
    if (!reportToken || reportToken !== source.reportToken) {
      return jsonError(res, 401, "Token de relatório inválido.");
    }

    const allowedStatuses = new Set(["running", "success", "error"]);
    const status = String(req.body?.status || "").toLowerCase();
    if (!allowedStatuses.has(status)) {
      return jsonError(res, 400, "Status de execução inválido.");
    }

    const phase = String(req.body?.phase || "runtime").slice(0, 40);
    const errorText = req.body?.error == null ? null : String(req.body.error).slice(0, 12000);
    const traceback = req.body?.traceback == null ? null : String(req.body.traceback).slice(0, 40000);
    const explicitLine = Number(req.body?.line);
    const line = Number.isInteger(explicitLine) && explicitLine > 0
      ? explicitLine
      : (extractErrorLine(traceback) || extractErrorLine(errorText));
    ...
    const event = {
      ...
      line: line || null,
      error: errorText,
      executor: req.body?.executor == null ? null : String(req.body.executor).slice(0, 200),
      placeId: req.body?.placeId == null ? null : String(req.body.placeId).slice(0, 100),
      jobId: req.body?.jobId == null ? null : String(req.body.jobId).slice(0, 200)
    };

    source.runtimeStatus = status;
    source.lastRuntimeAt = now;
    source.lastRuntimeLine = line || null;
    source.lastRuntimeError = status === "error" ? errorText : null;
    source.runtimeHistory = Array.isArray(source.runtimeHistory) ? source.runtimeHistory : [];
    source.runtimeHistory.unshift(event);
    source.runtimeHistory = source.runtimeHistory.slice(0, 100);

    if (status === "success" || status === "error") {
      const outputName = `${safeStem(source.filename)}_diagnostico.txt`;
      const storedName = `${source.id}_${Date.now()}_${outputName}`;
      const body = [
        "CAFEINA - Diagnóstico de execução",
        `Arquivo: ${source.filename}`,
        `Fonte: ${source.rawUrl}`,
        `Data: ${now}`,
        `Status: ${status}`,
        `Fase: ${phase}`,
        `Linha: ${line || "não identificada"}`,
        `Executor: ${event.executor || "não informado"}`,
        `PlaceId: ${event.placeId || "não informado"}`,
        `JobId: ${event.jobId || "não informado"}`,
        "",
      ...
      await fsp.writeFile(path.join(DIAG_DIR, storedName), `${body}\n`, "utf8");
      source.lastDownloadUrl = `${publicBase(req)}/api/diagnostics/${encodeURIComponent(storedName)}/download`;
      source.lastDiagnosticAt = now;
    }

    await saveSources(sources);
    return res.json({ ok: true, status, line: line || null });
  } catch (error) {
    console.error("diagnostic-sources/runtime-report", error);
    return jsonError(res, 500, "Não foi possível registrar o diagnóstico de execução.");
  }
});
...
app.use((error, _req, res, _next) => {
  ...
  if (error instanceof SyntaxError && "body" in error) return jsonError(res, 400, "JSON inválido na requisição.");
