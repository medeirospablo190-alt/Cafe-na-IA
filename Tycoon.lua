...
local function GetText(object)
    ...
    table.sort(candidates, function(a, b)
        ...
        if string.match(a, "%d") then aScore += 2 end
        if contains(a, "$") or contains(a, "buy") or contains(a, "point") then aScore += 3 end
        if string.match(b, "%d") then bScore += 2 end
        if contains(b, "$") or contains(b, "buy") or contains(b, "point") then bScore += 3 end
...
for _, name in ipairs({"CafeinaTycoonV2", "CafeinaTycoonV25", "CafeinaTycoonV3"}) do
...
Pages.Principal = function()
    ...
    InfoCard("TYCOON", tycoon and tycoon.Name or "Não detectado")
    InfoCard("COMPRAS", #purchases)
    InfoCard("COLETORES", #collectors)
    InfoCard("DROPPERS", #droppers)
    InfoCard("UPGRADES / REFINERS", #upgrades)
    InfoCard("REBIRTH", rebirth.State)
    InfoCard("AÇÃO", Runtime.Action)
...
Pages.Scanner = function()
    ...
    local tycoon = GetTycoonRoot(true)
    if not tycoon then
        Label("Tycoon não encontrado.", 50)
        return
    end
    ...
    InfoCard("TYCOON", tycoon:GetFullName())
    InfoCard("COMPRAS", #purchases)
    InfoCard("COLETORES", #collectors)
    InfoCard("DROPPERS", #droppers)
    InfoCard("UPGRADES", #upgrades)
    InfoCard("TOOLS", #tools)
    ...
    Button("📋 COPIAR DIAGNÓSTICO", function()
        ...
        for _, item in ipairs(purchases) do
            table.insert(report.purchases, {
                name = item.Name,
                display = item.Display,
                price = item.Price,
                touch = item.Touch:GetFullName(),
            })
        end

        for _, item in ipairs(collectors) do
            table.insert(report.collectors, {
                name = item.Name,
                touch = item.Touch:GetFullName(),
            })
        end
...
OpenPage("Principal")
print("[CAFEÍNA V3] Tycoon Mobile iniciado")
