...
local function refreshInfo()
    ...
    scanSizeLabel.Text = string.format("SCAN • %.1f MB / 150 MB", mb)
end
...
local function buildExport()
    return {
        schemaVersion = 4,
        scanner = "CAFEINA SMART SCANNER V4.1",
        generatedAt = os.time(),
...
local function scannerLoop()
    ...
    task.spawn(function()
        ...
        if STATE.ApproxBytes >= CONFIG.MaxBytesApprox then
            scanSizeLabel.Text = "SCAN • 150.0 MB / 150 MB"
        end
    end)
end
...
local function sendExport()
    ...
    task.spawn(function()
        ...
        local startOk, startResult = postJson(
            ...
            withToken({
                filename = filename,
                source = "cafeina-smart-scanner-v4.1",
                metadata = {
                    placeId = game.PlaceId,
                    gameId = game.GameId,
                    jobId = game.JobId,
