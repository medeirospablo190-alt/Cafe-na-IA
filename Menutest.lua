...
-- • PARAR TUDO
--
-- • MappingEngine focado em dados relevantes
-- • TestLab passivo/client-visible
-- • Scan + TestLab podem trabalhar juntos
-- • Limite total aproximado: 150 MB
-- • Upload em chunks
-- • /upload/start
-- • /upload/chunk
-- • /upload/finish
...
local CONFIG = {
    ...
    MAX_TEST_TRACKED_VALUES =
        2500,

    MAX_TEST_TRACKED_ATTRIBUTES =
        1500,

    MAX_TEST_DYNAMIC_OBJECTS =
        5000,
    ...
    ----------------------------------------------------------
    -- LIMITES DE CLASSES REPETITIVAS
    ----------------------------------------------------------
...
local function relevanceScore(inst)
    ...
    for _, keyword
        in ipairs(CONFIG.NAME_KEYWORDS)
    do
...
local function updateUploadUI()
    ...
    if Upload.Running then
        ...
        progress =
            math.clamp(
                Upload.BytesSent / total,
                0,
                1
            )
    elseif Upload.BytesSent > 0 then
        progress = 1
    end
    ...
    do
        ...
        do
            ...
            if
                inst:IsA("ValueBase")
                and countValues
                    < CONFIG.MAX_TEST_TRACKED_VALUES
            then

                TrackedValues[inst] =
                    snapshotValueObject(inst)
            ...
            if
                countAttributes
                < CONFIG.MAX_TEST_TRACKED_ATTRIBUTES
            then
...
function TestLab:StartObservers()
        ...
        local addedConnection =
            root.DescendantAdded:Connect(
                function(inst)
                    ...
                    if
                        dynamicCount
                        >= CONFIG.MAX_TEST_DYNAMIC_OBJECTS
                    then
                        return
                    end
    ...
    local removingPlayerConnection =
...
local function uploadReport()
    ...
    if url then
        ...
        LinkText.Text =
            "Upload concluído sem URL retornada"
    end
...
local function startFullSession()
    ...
    task.spawn(function()
        ...
        ------------------------------------------------------
        -- O scan terminou.
        -- Encerrar também o TestLab da mesma sessão.
        ------------------------------------------------------
...
print(
    "[CAFEÍNA] Mapping Engine V1 carregado."
)
