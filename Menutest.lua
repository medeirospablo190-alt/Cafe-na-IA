...
local CONFIG = {
    VERSION = "MAPPING_V1_6_UNIVERSAL_SMART_COMPACT",
...
local Flow = {
    Mode = "idle", -- idle | collecting | finalizing | uploading
...
local function buildUploadHeaderLine()
    return safeJson({
        ...
        metadata={
            scanner=CONFIG.VERSION,
            build="UNIVERSAL_SMART_PASSIVE_1",
            clientVisibleOnly=true,
            placeId=game.PlaceId,
            gameId=game.GameId,
            placeVersion=game.PlaceVersion,
...
local function uploadAll()
    ...
    if not okBuild or not totalChunks or totalChunks <= 0 then
        ...
        ActionButton.BackgroundColor3 = COLORS.BUTTON
        Flow.Mode = "idle"
        setUiMessage(StatusLabel.Text, string.format("%.2f MB continuam arquivados", mb(Archive.Bytes)))
        updateUI(true)
        return
    end
...
if Archive.Records > 0 then
    setUiMessage(
        ...
        string.format("%.2f MB preservados • nova coleta será adicionada", mb(Archive.Bytes))
    )
end
updateUI(true)
