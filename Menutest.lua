...
gui.Name = "CafeinaSmartScannerV41"
...
title.Text = "CAFEÍNA • SMART SCANNER V4.1"
...
local function buildExport()
    return {
        schemaVersion = 5,
        scanner = "CAFEINA SMART SCANNER V4.1 SELECTIVE",
        generatedAt = os.time(),
...
local function runPass()
    ...
    -- Ordem de prioridade baseada nas informações encontradas no jogo.
    scanRemotes()
    ...
    -- Pequeno resumo temporal para correlacionar mudanças entre passes.
...
local function scannerLoop()
    ...
    task.spawn(function()
        ...
        -- O scanner só encerra por interrupção manual ou limite de 150 MB.
        if STATE.ApproxBytes >= CONFIG.MaxBytesApprox then
...
local function sendExport()
    ...
    task.spawn(function()
        ...
        local startOk, startResult = postJson(
            ...
            withToken({
                filename = filename,
                source = "cafeina-smart-scanner-v4.1-selective",
                metadata = {
                    placeId = game.PlaceId,
                    gameId = game.GameId,
                    jobId = game.JobId,
