...
OBSERVAÇÃO
- Upload já configurado para https://cafe-na-ia.onrender.com.
- Usa /upload/start, /upload/chunk e /upload/finish.
...
local CONFIG = {
    ...
    Version = "V1.4 FOCUSED BALLISTICS",
    ...
    -- Backend CAFEÍNA.
    SiteBaseURL = "https://cafe-na-ia.onrender.com",

    -- Deixe vazio se UPLOAD_TOKEN não estiver configurado no Render.
    UploadToken = "",

    -- O backend atual aceita até 6 MB por chunk.
    -- Mantemos ~900 KB para reduzir pico de memória no executor/mobile.
    UploadChunkBytes = 900 * 1024,

    -- Pausa curta entre chunks para reduzir pico de CPU/memória.
    UploadChunkDelay = 0.10,

    -- Coleta focada: evita gastar memória com objetos irrelevantes.
    FocusedResearch = {
        Enabled = true,

        -- Estruturas mais importantes já descobertas no jogo.
        PriorityPaths = {
            "ReplicatedStorage.BlasterSystem.Blaster",
            "ReplicatedStorage.__remotes.TycoonService",
            "ReplicatedStorage.__remotes.Admins",
        ...
        PriorityNames = {
            ...
            "playSoundAtPosition",
            "BlasterExtension",
            "RaycastVisualizerClass",
        },

        -- Objetos transitórios genéricos que geravam muito lixo.
        IgnoreNames = {
            "OriginalPosition",
            "OriginalSize",
            "Debris",
        ...
        -- Trail/impact ainda são importantes quando associados a um tiro,
        -- então só são ignorados fora de uma janela de disparo.
        IgnoreGenericTrailOutsideShot = true,

        -- Limite para snapshots repetidos da mesma estrutura.
        SnapshotCooldown = 3.0,
    },
...
local function buildMetadata()
    return {
        ...
        focus = {
            ...
            "Shot timing / shot correlation",
            "Hitscan / raycast structure",
            "Projectile structure",
            "Wall/material impacts",
            "Piercing / penetration clues",
            "Impact decals/particles/sounds",
            "Trail / Beam / tracer effects",
            "Backpack changes",
...
--==============================================================
-- SAFE STREAMING UPLOAD • CAFEÍNA SERVER PROTOCOL
...
local function siteUrl(route)
    return string.gsub(CONFIG.SiteBaseURL or "", "/+$", "") .. route
end

local function buildFilename()
    return string.format(
        "Cafeina_WeaponBallistics_%s_%s.json",
        tostring(game.PlaceId),
        os.date("!%Y%m%d_%H%M%S")
    )
end

local function serverError(response, fallback)
    ...
    if status then
        return tostring(fallback) .. " • HTTP " .. tostring(status)
    end

    return tostring(fallback)
end

local function postPacket(route, packet)
    if CONFIG.UploadToken ~= "" then
        packet.token = CONFIG.UploadToken
    end

    local encoded, encodeErr = safeEncode(packet)

    if not encoded then
        return false, nil, "JSON: " .. tostring(encodeErr)
    end

    local ok, response = pcall(function()
        return postJson(siteUrl(route), encoded)
    end)
    ...
    if not responseSucceeded(response) then
        return false, response, serverError(response, "Falha em " .. route)
    end
    ...
    if type(decoded) ~= "table" or decoded.ok ~= true then
        return false, response,
            decoded and tostring(decoded.message or "Resposta inválida")
            or "Servidor retornou uma resposta inválida."
    end

    return true, response, decoded
end

local function startServerUpload()
    local packet = {
        filename = buildFilename(),
        source = "cafeina-weapon-ballistics-research-v1.4-focused",
        metadata = buildMetadata(),
    }

    packet.metadata.area = "WeaponBallistics"
    packet.metadata.clientVisibleOnly = true

    local ok, _, decodedOrError = postPacket("/upload/start", packet)

    if not ok then
        return false, decodedOrError
    end

    local uploadId = decodedOrError.uploadId
...
local function sendServerChunk(uploadId, chunkIndex, objects)
    -- O server.js atual exige exatamente:
    -- uploadId, index, objects
    local packet = {
        uploadId = uploadId,
        index = chunkIndex,
        objects = objects,
    ...
    local ok, _, decodedOrError = postPacket("/upload/chunk", packet)
...
local function finishServerUpload(uploadId, totalChunks, sentRecords)
    local packet = {
        uploadId = uploadId,
        totalChunks = totalChunks,

        summary = {
            area = "WeaponBallistics",
            scanner = CONFIG.Title,
            version = CONFIG.Version,
            records = sentRecords,
            passes = STATE.Pass,
            ...
            ballistics = CONFIG.Ballistics.Enabled,
            focusedResearch = CONFIG.FocusedResearch.Enabled,
        },
    }

    local ok, _, decodedOrError = postPacket("/upload/finish", packet)
...
local function cancelServerUpload(uploadId)
    if type(uploadId) ~= "string" or uploadId == "" then
        return
    end

    pcall(function()
        postPacket("/upload/cancel", {
            uploadId = uploadId,
        })
    end)
end
...
local function uploadReport()
    ...
    if type(CONFIG.SiteBaseURL) ~= "string"
    or CONFIG.SiteBaseURL == "" then
        setStatus(
            "URL do site não configurada",
    ...
    local ok, err = xpcall(function()
        ...
        local startOk, uploadIdOrError = startServerUpload()

        if not startOk then
            error("START: " .. tostring(uploadIdOrError))
        end

        activeUploadId = uploadIdOrError
        ...
        local chunkIndex = 0
        local sentRecords = 0
        local totalRecords = #STATE.Records
        ...
        local function flushCurrent()
            ...
            local chunkOk, decodedOrError = sendServerChunk(
                activeUploadId,
                chunkIndex,
                current
            )
            ...
            if not chunkOk then
                error(
                    ...
                    .. tostring(chunkIndex)
                    .. ": "
                    .. tostring(decodedOrError)
                )
            end

            sentRecords += #current

            -- Libera a tabela do chunk antes de continuar.
            current = {}
            currentBytes = 2
        ...
        -- Importante:
        -- nunca fazemos JSONEncode de STATE.Records inteiro.
        for i = 1, totalRecords do
            local record = STATE.Records[i]

            -- Apenas o registro individual é serializado para estimar
            -- o tamanho do próximo chunk.
            local encodedRecord, encodeErr = safeEncode(record)
            ...
            local recordBytes = #encodedRecord + 1
            encodedRecord = nil

            -- Registro isolado muito grande: deixa o backend responder
            -- claramente em vez de criar um relatório gigante na memória.
            if recordBytes > 5.5 * 1024 * 1024 then
                error(
                    "Registro "
                    .. tostring(i)
                    .. " excede o tamanho seguro de upload."
                )
            end
            ...
            -- Cede CPU para não congelar o cliente durante a separação.
            if i % 180 == 0 then
                RunService.Heartbeat:Wait()
            end
        end
        ...
        setStatus(
            string.format(
                "Finalizando • %d blocos...",
                chunkIndex
            )
        )

        local finishOk, finalOrError = finishServerUpload(
            activeUploadId,
            chunkIndex,
            sentRecords
        )
        ...
        if not finishOk then
            error("FINISH: " .. tostring(finalOrError))
        end

        activeUploadId = nil

        ----------------------------------------------------------
        -- 4. COPY ONLY FINAL LINK
        ----------------------------------------------------------
        local link = extractLink(finalOrError)

        if type(link) == "string" and link ~= "" then
            if copyText(link) then
                setStatus(
                    string.format(
                        "Enviado • %d registros • link copiado",
                        sentRecords
                    ),
    ...
    if not ok then
        ...
        -- Remove a sessão parcial no servidor quando possível.
        if activeUploadId then
            cancelServerUpload(activeUploadId)
        end
...
--==============================================================
-- FOCUSED BLASTER RESEARCH
--==============================================================

local FocusLastSnapshot = {}

local PRIORITY_NAME_SET = {}
for _, name in ipairs(CONFIG.FocusedResearch.PriorityNames) do
    PRIORITY_NAME_SET[string.lower(name)] = true
end

local IGNORE_NAME_SET = {}
for _, name in ipairs(CONFIG.FocusedResearch.IgnoreNames) do
    IGNORE_NAME_SET[string.lower(name)] = true
end

local function hasPriorityName(obj)
    if not obj then return false end
    return PRIORITY_NAME_SET[string.lower(obj.Name)] == true
end

local function ignoredGenericName(obj)
    if not obj then return false end
    return IGNORE_NAME_SET[string.lower(obj.Name)] == true
end

local function isUnderBlasterSystem(obj)
    if not obj then return false end
    ...
    while current and depth < 12 do
        if current.Name == "BlasterSystem"
        or current.Name == "Blaster" then
            return true
        end
...
local function snapshotCooldownKey(obj, kind)
    return tostring(kind)
        .. "|"
        .. safeFullName(obj)
end

local function canSnapshotNow(obj, kind)
    local key = snapshotCooldownKey(obj, kind)
    local now = os.clock()
    local last = FocusLastSnapshot[key] or -math.huge

    if now - last < CONFIG.FocusedResearch.SnapshotCooldown then
        return false
    end

    FocusLastSnapshot[key] = now
    return true
end

local function focusedObjectData(obj)
    local data = {
        name = obj.Name,
        class = obj.ClassName,
        path = safeFullName(obj),
        attributes = safeAttributes(obj),
        parent = obj.Parent and safeFullName(obj.Parent) or nil,
    }
    ...
    or obj:IsA("Script") then
        data.scriptLike = true
    end

    if obj:IsA("BasePart") then
        data.position = vectorTable and vectorTable(obj.Position) or nil
        data.size = vectorTable and vectorTable(obj.Size) or nil
...
local function recordPriorityObject(obj, reason)
    if not CONFIG.FocusedResearch.Enabled
    or not obj
    or ignoredGenericName(obj) then
        return
    end

    if not canSnapshotNow(obj, reason or "priority") then
        return
    end

    addRecord("BlasterResearch", "priority_object", {
        reason = reason,
        object = focusedObjectData(obj),
    })
end

local function scanPriorityStructure()
    if not CONFIG.FocusedResearch.Enabled then
        return
    end

    addRecord("Analysis", "focused_scan_started", {
        priorityNames = CONFIG.FocusedResearch.PriorityNames,
        priorityPaths = CONFIG.FocusedResearch.PriorityPaths,
    })
    ...
    for _, root in ipairs(roots) do
        ...
        for i, obj in ipairs(descendants) do
            if hasPriorityName(obj) then
                recordPriorityObject(obj, "priority_name")
            elseif isUnderBlasterSystem(obj) then
                local lower = string.lower(obj.Name)

                if string.find(lower, "ray", 1, true)
                or string.find(lower, "projectile", 1, true)
                or string.find(lower, "bullet", 1, true)
                or string.find(lower, "impact", 1, true)
                or string.find(lower, "pierce", 1, true)
                or string.find(lower, "trail", 1, true)
                or string.find(lower, "beam", 1, true)
                or string.find(lower, "sound", 1, true)
                or string.find(lower, "damage", 1, true) then
                    recordPriorityObject(obj, "blaster_structure")
                end
            end
    ...
    addRecord("Analysis", "focused_scan_finished", {
        at = unix(),
    })
end
...
local function beginShot(tool, source)
    ...
    addRecord("Ballistics", "shot", {
        ...
        classification = {
            possibleHitscan =
                shot.context
                and shot.context.probe
                and shot.context.probe.hit ~= nil
                or false,

            tool = tool and tool.Name or nil,
            fireMode = tool and safeAttr(tool, "fireMode", "FireMode") or nil,
            projectileType = tool and safeAttr(tool, "projectileType", "ProjectileType") or nil,
            range = tool and safeAttr(tool, "range", "Range") or nil,
            spread = tool and safeAttr(tool, "spread", "Spread") or nil,
            damage = tool and safeAttr(tool, "damage", "Damage") or nil,
            rateOfFire = tool and safeAttr(tool, "rateOfFire", "FireRate") or nil,
        },
    })
...
local function trackProjectile(part, shot)
    ...
    touchConnection = part.Touched:Connect(function(hit)
        ...
        local speedBefore = beforeVelocity.Magnitude
        ...
        addRecord("Ballistics", "projectile_touched", {
            ...
            hit = {
                ...
                transparency = hit:IsA("BasePart") and hit.Transparency or nil,
                canCollide = hit:IsA("BasePart") and hit.CanCollide or nil,
                canQuery = hit:IsA("BasePart") and hit.CanQuery or nil,
            },

            clues = {
                possiblePenetration = false,
                possibleRicochet = false,
            },
        })

        task.delay(0.03, function()
            if part and part.Parent then
                local afterVelocity = part.AssemblyLinearVelocity
                local afterSpeed = afterVelocity.Magnitude
                local beforeSpeed = beforeVelocity.Magnitude

                local directionDot = nil
                if beforeSpeed > 0.001 and afterSpeed > 0.001 then
                    directionDot = beforeVelocity.Unit:Dot(afterVelocity.Unit)
                end

                addRecord("Ballistics", "post_impact_velocity", {
                    shotId = shot and shot.id or nil,
                    projectile = safeFullName(part),
                    velocity = vectorTable(afterVelocity),
                    position = vectorTable(part.Position),

                    analysis = {
                        speedBefore = beforeSpeed,
                        speedAfter = afterSpeed,
                        retainedSpeedRatio =
                            beforeSpeed > 0.001
                            and (afterSpeed / beforeSpeed)
                            or nil,
                        ...
                        possiblePenetration =
                            afterSpeed > 5
                            and directionDot ~= nil
                            and directionDot > 0.65
                            or false,

                        possibleRicochet =
                            afterSpeed > 5
                            and directionDot ~= nil
                            and directionDot < 0.45
                            or false,
                    },
                })
            end
...
local function observeShotArtifact(obj)
    ...
    local artifactData = describeArtifact(obj)
    ...
    addRecord("Ballistics", "shot_artifact", {
        ...
        correlation = {
            isImpactLike =
                string.find(string.lower(obj.Name), "impact", 1, true) ~= nil
                or obj:IsA("Decal")
            ...
            isTrajectoryLike =
                obj:IsA("Beam")
                or obj:IsA("Trail")
                or string.find(string.lower(obj.Name), "tracer", 1, true) ~= nil
                or string.find(string.lower(obj.Name), "bullet", 1, true) ~= nil
                or string.find(string.lower(obj.Name), "projectile", 1, true) ~= nil,
        },
    })
...
local function startBallisticsObservers()
    ...
    Connections[#Connections + 1] = Workspace.DescendantAdded:Connect(function(obj)
        ...
        if hasPriorityName(obj)
        or (isUnderBlasterSystem(obj) and not ignoredGenericName(obj)) then
            recordPriorityObject(obj, "runtime_added")
        end
    end)
...
task.defer(function()
    local ok, err = pcall(scanPriorityStructure)
    if not ok then
        STATE.LastError = "Focused scan: " .. tostring(err)
        warn("[CAFEÍNA] Focused scan error: " .. tostring(err))
    end
end)
