...
-- Forward declaration: algumas rotinas de upload/finalização
-- são definidas antes do módulo de diagnóstico.
local Diagnostic
...
local function finalizeDiagnosticSuccess()
    ...
    if Diagnostic then
        ...
        -- O único relatório completo da execução é enviado aqui.
        Diagnostic.sendFull()
    end
...
create(
    "TextLabel",
    {
        ...
        Text =
            "Scamtest.lua • V12 ANDROID • otimizado",
...
function Diagnostic.sync()
    ...
    c.diagnosticLastHTTPStatus =
        Diagnostic.LastHTTPStatus

    c.diagnosticNetworkOk =
        Diagnostic.LastNetworkError == nil
end
...
-- Envia somente o estado atual + o evento novo.
-- Não reenvia todo o histórico a cada step.
function Diagnostic.sendIncremental(
    eventType,
    event
)
    if not Diagnostic.RunId then
        return
    end
    ...
    local report = {
        ...
        -- Mantém o formato do endpoint, mas apenas com o evento novo.
        steps =
            {},
...
-- Relatório completo é enviado apenas no encerramento.
function Diagnostic.sendFull()
    if not Diagnostic.RunId then
        return
    end
...
function Diagnostic.start()
    ...
    Diagnostic.Report.message =
        "Scamtest.lua V12 Android iniciado"
    ...
    task.spawn(function()
        while Diagnostic.Heartbeat do
            ...
            if
                ...
                >= CONFIG.DIAGNOSTIC_HEARTBEAT
            then
                -- Heartbeat compacto: só snapshot atual.
                Diagnostic.sendIncremental(
                    "heartbeat"
                )
...
function Diagnostic.step(
    ...
    -- O histórico completo continua localmente, com limite.
    while
...
function Diagnostic.error(
    ...
    -- Primeiro manda o erro compacto e depois um fechamento completo.
    Diagnostic.sendIncremental(
        "error",
        item
    )
...
print(
    "[CAFEINA] Scamtest.lua V12 Android carregado."
)
