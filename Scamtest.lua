...
local function finalizeDiagnosticSuccess()
    ...
    -- Para heartbeats/loops que respeitem o estado final.
    State.StopDiagnosticHeartbeat =
        true
    ...
    if setUploadProgress then
        setUploadProgress(1)
    end

    if setUploadDetail then
        setUploadDetail(
            "Servidor • upload concluído"
        )
    end
end

local function reportScanStoppedOnce(message)
    if State.ScanStoppedReported then
        return false
    end

    State.ScanStoppedReported =
        true

    if Diagnostic and Diagnostic.addStep then
        pcall(
            Diagnostic.addStep,
            "scan_stopped",
            message
...
create(
    "TextLabel",
    {
        ...
        Text =
            "Scamtest.lua • V9 • fluxo finalizado",
...
print(
    "[CAFEINA] Scamtest.lua V9 carregado."
)
