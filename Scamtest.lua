...
create(
    "TextLabel",
    {
        ...
        Text =
            "Scamtest.lua • V8 • diagnóstico copiável",
...
--==============================================================--
-- DIAGNÓSTICO LOCAL • COPIAR JSON
--==============================================================--

local function buildLocalDiagnosticSnapshot()
    local now =
        os.time()
    ...
    local counters = {
        ...
        objectsSeen =
            State.ObjectsSeen or 0,

        recordsStored =
            State.RecordsStored or 0,

        historyRecords =
            State.HistoryRecords or 0,

        newRecords =
            State.NewRecords or 0,

        updatedRecords =
            State.UpdatedRecords or 0,

        duplicatesFiltered =
            State.DuplicatesFiltered or 0,

        approxBytes =
            State.ApproxBytes or 0,

        realEncodedBytes =
            State.RealEncodedBytes or 0,
    ...
    -- Inclui os contadores de upload quando existirem.
    for _, key in ipairs({
        "UploadChunk",
        "UploadChunksSent",
        "UploadBytesSent",
        "UploadBytesTotal",
        "UploadRecordsProcessed",
        "UploadRecordsTotal",
        "UploadStage",
    }) do
        local value =
        ...
        if value ~= nil then
            local outputKey =
                key:gsub("^Upload", "upload")

            outputKey =
                outputKey:sub(1, 1):lower()
                .. outputKey:sub(2)
    ...
    -- Se o módulo de diagnóstico já mantém um relatório local,
    -- inclui uma cópia sem depender do servidor.
    if Diagnostic then
    ...
    if not ok then
        return nil,
            "Falha ao gerar JSON do diagnóstico"
    end
...
local function copyDiagnosticToClipboard()
    local encoded, err =
        buildLocalDiagnosticSnapshot()

    if not encoded then
        if setStatus then
            setStatus(
                err
                or "Erro ao gerar diagnóstico"
            )
        end
    ...
    if not ok then
        if setStatus then
            setStatus(
                "Falha ao copiar diagnóstico"
            )
        end

        warn(
            "[CAFEINA] Falha ao copiar diagnóstico:",
            copyErr
        )
...
print(
    "[CAFEINA] Scamtest.lua V8 carregado."
)
