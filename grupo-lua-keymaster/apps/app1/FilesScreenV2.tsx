...
export function FilesScreen({ sessionToken, deviceToken }: {
  ...
  const shareCommentChars = Array.from(shareComment).length;
  ...
  async function saveEditor() {
    ...
    if (editorBytes > MAX_TEXT_BYTES) {
      Alert.alert("Arquivo muito grande", `O limite é ${bytesLabel(MAX_TEXT_BYTES)} por código/loadstring.`);
      return;
    }
  ...
  function confirmDelete(ids: string[]) {
    ...
    Alert.alert(
      count === 1 ? "Apagar arquivo" : `Apagar ${count} arquivos`,
      "A exclusão é permanente e também remove publicações do feed que dependam desse arquivo.",
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert("Não foi possível apagar", error instanceof Error ? error.message : "Erro desconhecido.");
            } finally {
              setMutationBusy(false);
            }
          }
  ...
  async function submitShare() {
    if (!shareTarget || mutationBusy) return;
    setMutationBusy(true);
    try {
      await shareLibraryItemWithComment(sessionToken, deviceToken, shareTarget.id, shareComment.trim());
      const title = shareTarget.title;
      setShareTarget(null);
      setShareComment("");
      setSelectedIds([]);
      setMessage(`${title} foi compartilhado no feed.`);
      await reload();
    } catch (error) {
  ...
  return (
    <View style={local.root}>
        ...
        <>
          ...
          <View style={local.searchRow}>
            ...
            <Pressable
              ...
              accessibilityLabel="Mostrar somente favoritos"
            >
            ...
            <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={local.bulkActions}>
                ...
                <Pressable style={[local.bulkButton, local.bulkDanger]} disabled={mutationBusy} onPress={() => confirmDelete(selectedIds)}>
                  <Text style={local.bulkDangerText}>APAGAR</Text>
                </Pressable>
              ...
              >
                ...
                {!selectionMode ? (
                  <View style={local.actions}>
                    ...
                    <Pressable
                      style={[local.action, item.favorite && local.favoriteAction]}
                    ...
                    >
                      <Text style={[local.actionText, item.favorite && local.favoriteActionText]}>
                        {item.favorite ? "★ FAVORITO" : "☆ FAVORITAR"}
                      </Text>
                    </Pressable>
                    <Pressable style={local.action} onPress={(event) => { event.stopPropagation(); openShare(item); }}>
                      <Text style={local.actionText}>FEED</Text>
                    </Pressable>
                    <Pressable style={[local.action, local.deleteAction]} onPress={(event) => { event.stopPropagation(); confirmDelete([item.id]); }}>
                      <Text style={local.deleteActionText}>APAGAR</Text>
                    </Pressable>
                  </View>
                ) : null}
              </Pressable>
              ...
              {editor ? (
                <>
                  ...
                  <View style={local.editorActions}>
                    ...
                    <Pressable
                      style={[local.saveButton, (editorBusy || editorBytes > MAX_TEXT_BYTES) && local.disabled]}
                      disabled={editorBusy || editorBytes > MAX_TEXT_BYTES}
                      onPress={saveEditor}
                    >
      ...
      <Modal visible={Boolean(shareTarget)} transparent animationType="fade" onRequestClose={() => !mutationBusy && setShareTarget(null)}>
        <KeyboardAvoidingView style={local.shareBackdrop} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          <View style={local.shareModal}>
            <View style={local.editorHeader}>
              ...
              <Pressable disabled={mutationBusy} onPress={() => setShareTarget(null)}>
                <Text style={local.close}>✕</Text>
              </Pressable>
            </View>
            <Text style={local.shareHint}>Adicione um comentário opcional. O código/loadstring continua sendo copiado separadamente no feed.</Text>
            <TextInput
              value={shareComment}
            ...
            <View style={local.editorActions}>
              <Pressable style={local.secondaryButton} disabled={mutationBusy} onPress={() => { setShareTarget(null); setShareComment(""); }}>
              ...
              <Pressable style={[local.saveButton, mutationBusy && local.disabled]} disabled={mutationBusy} onPress={submitShare}>
                <Text style={local.saveButtonText}>{mutationBusy ? "PUBLICANDO..." : "COMPARTILHAR"}</Text>
              </Pressable>
            </View>
          </View>
        </KeyboardAvoidingView>
...
const local = StyleSheet.create({
  ...
  action: { flexGrow: 1, minWidth: 76, borderRadius: 9, borderWidth: 1, borderColor: "#2A2A30", paddingVertical: 8, paddingHorizontal: 7, alignItems: "center" },
  actionText: { color: "#CFCFD3", fontWeight: "900", fontSize: 8 },
  favoriteAction: { borderColor: "#7A6022", backgroundColor: "#171205" },
  favoriteActionText: { color: "#FFD35A" },
  deleteAction: { borderColor: "#502126", backgroundColor: "#130708" },
  deleteActionText: { color: "#FF6258", fontWeight: "900", fontSize: 8 },
  ...
  loadstringEditorModal: { height: "68%" },
  ...
  shareBackdrop: { flex: 1, backgroundColor: "rgba(0,0,0,0.88)", justifyContent: "center", padding: 18 },
  shareModal: { borderRadius: 20, borderWidth: 1, borderColor: "#30263A", backgroundColor: "#09090C", padding: 16 },
  ...
  shareInput: { minHeight: 118, maxHeight: 190, borderRadius: 13, borderWidth: 1, borderColor: "#29292F", backgroundColor: "#0D0D11", color: "#FFFFFF", padding: 12, fontSize: 13 },
