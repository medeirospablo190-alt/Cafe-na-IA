...
import { listFeedPosts, type FeedPost } from "./api";
  ...
  return (
    <View style={local.root}>
      <View style={local.header}>
        ...
        <Pressable style={local.refresh} onPress={() => reload()}>
        ...
        <View key={post.id} style={local.post}>
          <View style={local.postHeader}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={local.meta}>{formatDate(post.createdAt)}</Text>
            </View>
          ...
          <Pressable style={local.copyButton} onPress={() => Clipboard.setStringAsync(post.item.content)}>
            <Text style={local.copyText}>COPIAR {post.kind === "CODE" ? "CÓDIGO" : "LOADSTRING"}</Text>
          </Pressable>
        </View>
      )) : null}
    </View>
...
const local = StyleSheet.create({
  ...
  message: { color: "#FF8C82", fontSize: 11, marginTop: 10 },
