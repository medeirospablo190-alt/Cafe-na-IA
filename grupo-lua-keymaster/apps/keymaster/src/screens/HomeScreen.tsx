    ...
    listManagedMenus(session)
      .then((result) => {
        ...
        setMenuSummary({
          ...
          accessesMonth: result.menus.reduce((sum, menu) => sum + Number(menu.accesses_month || 0), 0)
        });
      })
        ...
        <ScrollView contentContainerStyle={styles.pageWithNav}>
          ...
          <View style={styles.metricGrid}>
            ...
            <View style={styles.metricCard}>
              ...
              <Text style={styles.metricValue}>{menuSummary ? menuSummary.free + menuSummary.vip : "—"}</Text>
              <Text style={styles.metricHint}>{menuSummary ? `${menuSummary.free} FREE • ${menuSummary.vip} VIP` : "carregando"}</Text>
            </View>
            <View style={styles.metricCard}>
              <Text style={styles.metricLabel}>SESSÕES APP 1</Text>
          ...
          <Pressable style={styles.featureCard} onPress={onMenus}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.muted}>
                {menuSummary
                  ? `${menuSummary.total} menus • ${menuSummary.free} FREE • ${menuSummary.vip} VIP • ${menuSummary.accessesMonth} acessos neste mês.`
                  : "Cadastrar fontes .lua, gerar URLs de acesso e administrar autorizações por menu."}
              </Text>
            </View>
            <Text style={styles.cardArrow}>›</Text>
          </Pressable>
